        ...
        <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <div>
                ...
                <label for="item_price" class="block text-sm mt-4">
                    ...
                    <input id="item_price" name="item_price" type="number" step="0.01" value="{{ $product->item_price }}"
                        class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                ...
                <label for="item_type" class="block mt-4 text-sm">
                    ...
                    <select id="item_type" name="item_type"
                        ...
                        <option value="Phone Case" @if ($product->item_type === 'Phone Case') selected @endif>Phone Case</option>
                        <optgroup label="Glass">
                            <option value="Glass Privacy" @if ($product->item_type === 'Glass Privacy') selected @endif>Privacy</option>
                            <option value="Glass Clear" @if ($product->item_type === 'Glass Clear') selected @endif>Clear</option>
                            <option value="Glass Matte" @if ($product->item_type === 'Glass Matte') selected @endif>Matte</option>
                            <option value="Glass Silicon" @if ($product->item_type === 'Glass Silicon') selected @endif>Silicon</option>
                        </optgroup>
                        <optgroup label="GADGETS">
                            <option value="Charger" @if ($product->item_type === 'Charger') selected @endif>Charger</option>
                            <option value="Wire" @if ($product->item_type === 'Wire') selected @endif>Wire</option>
